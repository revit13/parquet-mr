...
class ColumnChunkPageReadStore implements PageReadStore, DictionaryPageReadStore {
  ...
  static final class ColumnChunkPageReader implements PageReader {
    ...
    ColumnChunkPageReader(String[] columnPath, BytesDecompressor decompressor, List<DataPage> compressedPages, DictionaryPage compressedDictionaryPage,
        BlockCipher.Decryptor blockDecryptor, byte[] aadPrefix, short rowGroupOrdinal, short columnOrdinal) {
      ...
      if (null != blockDecryptor) {
        dataPageAAD = AesEncryptor.createModuleAAD(aadPrefix, AesEncryptor.DataPage, rowGroupOrdinal, columnOrdinal, pageOrdinal);
        dictionaryPageAAD = AesEncryptor.createModuleAAD(aadPrefix, AesEncryptor.DictionaryPage, rowGroupOrdinal, columnOrdinal, (short) -1);
      } 
    }
    ...
    public long getTotalValueCount() {
      if (hiddenColumn) return -1; // TODO Change Spark to explicitly support hidden columns.
        //throw new HiddenColumnException(columnPath);
      return valueCount;
    }
    ...
    public DictionaryPage readDictionaryPage() {
      if (hiddenColumn) return null; // TODO Change Spark to explicitly support hidden columns.
        //throw new HiddenColumnException(columnPath);
